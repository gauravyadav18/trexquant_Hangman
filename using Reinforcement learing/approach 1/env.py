...
# - Use spaces.MultiDiscrete and spaces.MultiBinary correctly (no .shape assignment)
# - Minor defensive checks / clearer nvec values for MultiDiscrete
...
class HangmanEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # load words file (expects words_250000_train.txt copied/linked as words.txt)
        f = open("./words.txt", 'r').readlines()
        self.wordlist = [w.strip() for w in f if w.strip()]

        # action space: choose one of 26 letters
        self.action_space = spaces.Discrete(26)

        # vectorizer to represent actions-used vector (fits on alphabet)
        self.vectorizer = CountVectorizer(tokenizer=lambda x: list(x))
        self.vectorizer.fit([string.ascii_lowercase])

        self.config = config

        # mapping chars -> ids: a..z -> 0..25, '_' -> 26 (unknown)
